...
const OurServices = () => {
  ...
  const handleServiceClick = (title) => {
    ...
    navigate(`/service-details/${formattedTitle}`);
  };
    ...
    <section className="bg-gray-900 text-white py-16 px-8" id="service">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
        {services.map((service, index) => (
          <motion.div
            key={index}
            className={`bg-gradient-to-r ${service.gradient} p-6 rounded-2xl shadow-lg cursor-pointer`}
          ...
          >
            ...
            <span className="text-white font-semibold mt-4 inline-block hover:underline">Learn More →</span>
          </motion.div>
        ))}
      </div>
    </section>
